...
CORE_CHECKS = [
    ...
    ("Config Validation", ".agent/skills/lint-and-validate/scripts/config_validator.py", True),
    ("Lint Check", ".agent/skills/lint-and-validate/scripts/lint_runner.py", False),
    ("Type Check", ".agent/skills/lint-and-validate/scripts/type_coverage.py", False),
]
...
def run_script(name: str, script_path: Path, project_path: str, url: Optional[str] = None) -> dict:
    ...
    Returns:
        dict with keys: name, passed, output, error, skipped
    """
    if not check_script_exists(script_path):
        print_warning(f"{name}: Script not found, skipping")
        return {"name": name, "passed": True, "output": "", "error": "", "skipped": True}
    ...
    try:
        ...
        print_error(f"{name}: TIMEOUT (>60 minutes)")
...
def save_json_report(results: List[dict], report_path: Path):
    """Save results to a JSON file for Trifecta automation"""
    failed_checks = [r["name"] for r in results if not r["passed"] and not r.get("skipped")]
    error_logs = {r["name"]: r["error"] for r in results if not r["passed"] and not r.get("skipped") and r.get("error")}
